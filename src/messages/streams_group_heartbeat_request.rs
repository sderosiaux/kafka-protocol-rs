...
impl Encodable for CopartitionGroup {
    ...
    fn compute_size(&self, version: i16) -> Result<usize> {
        ...
        total_size += types::CompactArray(types::Int16).compute_size(&self.repartition_source_topics)?;
        let num_tagged_fields = self.unknown_tagged_fields.len();
        if num_tagged_fields > std::u32::MAX as usize {
            bail!("Too many tagged fields to encode ({} fields)", num_tagged_fields);
        }
...
impl Encodable for Subtopology {
    ...
    fn compute_size(&self, version: i16) -> Result<usize> {
        ...
        total_size += types::CompactArray(types::CompactString).compute_size(&self.source_topic_regex)?;
        ...
        total_size += types::CompactArray(types::Struct { version }).compute_size(&self.copartition_groups)?;
        let num_tagged_fields = self.unknown_tagged_fields.len();
        if num_tagged_fields > std::u32::MAX as usize {
            bail!("Too many tagged fields to encode ({} fields)", num_tagged_fields);
        }
