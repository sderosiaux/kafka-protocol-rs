...
impl Encodable for EndpointToPartitions {
    ...
    fn compute_size(&self, version: i16) -> Result<usize> {
        ...
        total_size += types::CompactArray(types::Struct { version }).compute_size(&self.active_partitions)?;
        total_size += types::CompactArray(types::Struct { version }).compute_size(&self.standby_partitions)?;
        let num_tagged_fields = self.unknown_tagged_fields.len();
        if num_tagged_fields > std::u32::MAX as usize {
            bail!("Too many tagged fields to encode ({} fields)", num_tagged_fields);
        }
...
impl StreamsGroupHeartbeatResponse {
    ...
    /// 
    /// Supported API versions: 0
    pub fn with_partitions_by_user_endpoint(mut self, value: Option<Vec<EndpointToPartitions>>) -> Self
    {
...
impl Encodable for StreamsGroupHeartbeatResponse {
    fn encode<B: ByteBufMut>(&self, buf: &mut B, version: i16) -> Result<()> {
        ...
        types::CompactArray(types::Struct { version }).encode(buf, &self.partitions_by_user_endpoint)?;
        let num_tagged_fields = self.unknown_tagged_fields.len();
        if num_tagged_fields > std::u32::MAX as usize {
            bail!("Too many tagged fields to encode ({} fields)", num_tagged_fields);
        }
    ...
    fn compute_size(&self, version: i16) -> Result<usize> {
        ...
        total_size += types::CompactArray(types::Struct { version }).compute_size(&self.partitions_by_user_endpoint)?;
        let num_tagged_fields = self.unknown_tagged_fields.len();
        if num_tagged_fields > std::u32::MAX as usize {
            bail!("Too many tagged fields to encode ({} fields)", num_tagged_fields);
        }
...
impl Decodable for StreamsGroupHeartbeatResponse {
    fn decode<B: ByteBuf>(buf: &mut B, version: i16) -> Result<Self> {
        ...
        let partitions_by_user_endpoint = types::CompactArray(types::Struct { version }).decode(buf)?;
        let mut unknown_tagged_fields = BTreeMap::new();
        let num_tagged_fields = types::UnsignedVarInt.decode(buf)?;
        for _ in 0..num_tagged_fields {
            let tag: u32 = types::UnsignedVarInt.decode(buf)?;
